...
const NAV = [
  ...
  { label: 'Points par Tâche', to: '/teacher/task-points', icon: Star },
    ...
    <div className="flex h-screen overflow-hidden bg-gradient-to-br from-pink-50 via-purple-50 to-indigo-50">
      ...
      <aside className={`flex flex-col h-full bg-white/80 backdrop-blur-md border-r border-pink-100 transition-all duration-300 shadow-lg ${collapsed ? 'w-16' : 'w-64'}`}>
        ...
        <div className="flex items-center gap-3 px-4 h-16 border-b border-pink-100">
          ...
          {!collapsed && <p className="text-sm font-semibold bg-gradient-to-r from-pink-600 to-purple-600 bg-clip-text text-transparent">MySchool Teacher</p>}
        </div>
        ...
        <div className="border-t border-pink-100 p-3">
          <div className={`flex items-center gap-3 px-3 py-2 rounded-xl ${!collapsed ? 'mb-2' : 'justify-center'}`}>
            <div className="w-8 h-8 rounded-full bg-gradient-to-r from-pink-400 via-purple-400 to-indigo-400 flex items-center justify-center text-xs font-bold text-white shadow-md">
              {user?.name?.[0]?.toUpperCase()}
            </div>
            {!collapsed && (
              <div className="flex-1 min-w-0">
                <p className="text-xs font-medium text-gray-700 truncate">{user?.name}</p>
                <p className="text-[10px] text-gray-400">Enseignant</p>
              </div>
            )}
          </div>
          <button
